      ...
      {data?.length > 0 && (
        <div className="py-8 ">
          ...
          <div className="grid lg:ml-6 md:mt-16 lg:mt-8 grid-cols-1 md:grid-cols-2 gap-16 lg:gap-x-32 2xl:grid-cols-3 gap-y-4">
            {data.map((item, index) => (
              <div onClick={()=>router?.push(`/profile/${item?.user_id}`)} key={index} className={`relative mb-6 cursor-pointer flex flex-col items-center h-fit lg:h-40 py-6 px-4 min-w-[70%] bg-white shadow-md rounded-sm mt-6 md:mt-2`}>
