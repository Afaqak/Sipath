...
const PasswordInput = ({ name, control, defaultValue, rules, placeholder, disabled, errors }) => {
  return (
    <Controller
      name={name}
      control={control}
      ...
      render={({ field }) => (
        <>
          <input
            ...
            className="shadow-[inset_2px_1px_6px_rgba(0,0,0,0.2)] w-full rounded-md px-3 py-1 placeholder:text-sm border-none focus:outline-none"
          />
          {errors && (
...
// export function PasswordInput({ placeholder, name,value, disabled, onChange }) {
